...
DROP TABLE &SCHEMAOWNER.blkEvntsToProcess cascade CONSTRAINTS;
...
CREATE GLOBAL TEMPORARY TABLE &SCHEMAOWNER.blkEvntsToProcess (BLK_RCRD_HDR_ID NUMBER) ON COMMIT PRESERVE ROWS;

CREATE OR REPLACE PROCEDURE &SCHEMAOWNER.dataMigrationFrom2020300To2021100_Bulk
(
...
AS
	...
	lv_blkEvntsToProcessReords 	NUMBER := 0;
	lv_blkHdrToProcessReords 	NUMBER := 0;
BEGIN
	...
	LOOP
		BEGIN
			...
			INSERT INTO &SCHEMAOWNER.MS_BLK_HDR (BLK_HDR_ID,CRTD_AT,LST_UPDT_AT,CRNT_STATUS,SRVC_TYPE,SRVC_NAME,SRVC_VER,SRVC_INST,SRVC_HOST,END_PNT_TYPE,END_PNT_NAME,MDL_TYPE,BLK_ID,BLK_TYPE,BLK_VER,BLK_SRC,BLK_LOC,BLK_FORMAT,BLK_SRC_ID)
			SELECT BLK_HDR_ID,CRTD_AT,LST_UPDT_AT,CRNT_STATUS,SRVC_TYPE,SRVC_NAME,SRVC_VER,SRVC_INST,SRVC_HOST,END_PNT_TYPE,END_PNT_NAME,MDL_TYPE,BLK_ID,BLK_TYPE,BLK_VER,BLK_SRC,BLK_LOC,BLK_FORMAT,BLK_SRC_ID
			FROM &SOURCE_USER.MS_BLK_HDR
			...
			INSERT into &SCHEMAOWNER.MS_BLK_EVNT (BLK_EVNT_ID,BLK_HDR_ID,STATUS,CRTD_AT,BTCH_ID,BTCH_TASK,BTCH_TOT_REC,BTCH_PRC_REC,BTCH_SUC_REC,BTCH_FLD_REC,ERR_CODE,ERR_DESC,CONNECT_ERR)
			SELECT BLK_EVNT_ID,BLK_HDR_ID,STATUS,CRTD_AT,BTCH_ID,BTCH_TASK,BTCH_TOT_REC,BTCH_PRC_REC,BTCH_SUC_REC,BTCH_FLD_REC,ERR_CODE,ERR_DESC,CONNECT_ERR
			...
			SELECT BLK_RCRD_HDR_ID,BLK_HDR_ID,CRTD_AT,LST_UPDT_AT,LST_TASK,CRNT_STATUS,RCRD_ID,SEQ_NO,RCRD_HDR_BODY
			...
			INSERT INTO &SCHEMAOWNER.blkEvntsToProcess(BLK_RCRD_HDR_ID)
			SELECT BLK_RCRD_HDR_ID FROM &SOURCE_USER.MS_RCRD_HDR RH
			Where RH.BLK_HDR_ID IN (SELECT BLK_HDR_ID FROM &SCHEMAOWNER.blkHdrToProcess);
			
			lv_rowCount := SQL%ROWCOUNT;

			-- Print body keys with total time elapsed
			lv_EndTime := SYSDATE;
			dbms_output.put_line('Loaded ' || CAST(lv_rowCount AS VARCHAR2) || ' bulk record event keys to Migrate. Total time elapsed: ' ||  CAST((lv_StartTime - lv_EndTime)*24*60*60 AS VARCHAR2) || ' secs.') ;
			
			INSERT INTO &SCHEMAOWNER.MS_RCRD_EVNT (BLK_RCRD_EVNT_ID,RCRD_HDR_ID,TASK_NAME,TASK_ID,STATUS,CRTD_AT,ERR_CODE,ERR_DESC,CONNECT_ERR)
			SELECT BLK_RCRD_EVNT_ID,RCRD_HDR_ID,TASK_NAME,TASK_ID,STATUS,CRTD_AT,ERR_CODE,ERR_DESC,CONNECT_ERR
			FROM &SOURCE_USER.MS_RCRD_EVNT EVNT
			WHERE EVNT.RCRD_HDR_ID IN (SELECT BLK_RCRD_HDR_ID FROM &SCHEMAOWNER.blkEvntsToProcess);
